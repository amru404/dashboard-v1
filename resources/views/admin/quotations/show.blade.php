...
@section('title', 'Quotation: ' . $quotation->display_name)
    ...
    <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('admin.quotations.index') }}" class="text-vd-primary hover:text-vd-primary/80 transition-colors">
                    ← Back
                </a>
            </div>
            <h1 class="text-3xl font-bold text-white mb-2">{{ $quotation->display_name }}</h1>
        ...
        <div class="flex gap-2 shrink-0">
            ...
            <form method="POST" action="{{ route('admin.quotations.destroy', $quotation) }}" onsubmit="return confirm('Delete this quotation?')" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="inline-flex items-center px-4 py-2.5 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400 font-semibold text-sm border border-red-500/30 transition-colors">
                    Delete
                </button>
            </form>
        </div>
...
<div class="grid gap-6 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2 space-y-6">
                ...
                @if ($quotation->file_path)
                    <div class="flex items-start gap-4 p-4 bg-[#0f1829]/50 rounded-lg border border-[#2a3f5f]">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-white truncate">{{ $quotation->original_filename }}</p>
                            <p class="text-xs text-gray-400 mt-1">{{ $quotation->mime_type }}</p>
                            <p class="text-sm text-gray-400 mt-2">
                                Size: <span class="font-semibold">{{ number_format($quotation->file_size / 1024 / 1024, 2) }} MB</span>
                            </p>
                        </div>
                        <a href="{{ route('admin.quotations.download', $quotation) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-vd-primary hover:bg-vd-primary/90 text-white font-semibold text-xs transition-colors shrink-0">
                            Download
                        </a>
                    </div>
                @else
        ...
        <div class="vd-card border-[#2a3f5f] !p-6">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-[#2a3f5f]">
                            @foreach ($quotation->users as $user)
            ...
            @else
                <div class="p-4 bg-gray-500/10 rounded-lg border border-gray-500/20">
                    <p class="text-sm text-gray-400">No users assigned to this quotation</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="vd-card border-[#2a3f5f] !p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-sm font-semibold text-white capitalize">{{ $quotation->status }}</p>
                </div>
        ...
        <div class="vd-card border-[#2a3f5f] !p-6">
            ...
            <div class="space-y-2">
                <a href="{{ route('admin.quotations.edit', $quotation) }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 rounded-lg bg-vd-primary/20 hover:bg-vd-primary/30 text-vd-primary font-semibold text-sm border border-vd-primary/30 transition-colors">
                    Edit Quotation
                </a>
                @if ($quotation->file_path)
                    <a href="{{ route('admin.quotations.download', $quotation) }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 rounded-lg bg-green-500/20 hover:bg-green-500/30 text-green-400 font-semibold text-sm border border-green-500/30 transition-colors">
                        Download File
                    </a>
                @endif
            </div>
