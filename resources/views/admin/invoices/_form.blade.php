@props(['invoice' => null, 'users' => []])

<form method="POST" action="{{ $invoice ? route('admin.invoices.update', $invoice) : route('admin.invoices.store') }}" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @if ($invoice)
        @method('PUT')
    @endif
    ...
    <div class="vd-card border-vd-formBorder !p-6">
        ...
        <div class="mb-6">
            ...
            <x-form-input id="display_name" name="display_name" type="text" value="{{ $invoice?->display_name ?? old('display_name') }}" required class="mt-2" />
                ...
                @forelse ($users as $user)
                    <label class="flex items-center gap-3 cursor-pointer hover:bg-white/5 p-2 rounded transition-colors">
                        <input type="checkbox" name="user_ids[]" value="{{ $user->id }}"
                               @checked($invoice && $invoice->users->contains($user))
        ...
        <div class="mb-6">
            ...
            @if ($invoice?->file_path)
                <p class="mt-2 text-xs text-gray-400">Current file: <span class="text-vd-primary">{{ $invoice->original_filename }}</span></p>
            @endif
        ...
        <div class="grid gap-6 sm:grid-cols-2 mb-6">
            <div>
                ...
                <select id="status" name="status" required class="vd-input mt-2">
                    <option value="draft" @selected($invoice?->status === 'draft' || old('status') === 'draft')>Draft</option>
                    <option value="sent" @selected($invoice?->status === 'sent' || old('status') === 'sent')>Sent</option>
                    <option value="paid" @selected($invoice?->status === 'paid' || old('status') === 'paid')>Paid</option>
                    <option value="cancelled" @selected($invoice?->status === 'cancelled' || old('status') === 'cancelled')>Cancelled</option>
                </select>
            ...
            <div>
                ...
                <x-form-input id="download_expired_at" name="download_expired_at" type="date" value="{{ $invoice?->download_expired_at?->format('Y-m-d') ?? old('download_expired_at') }}" class="mt-2 color-scheme-dark" />
        ...
        <div class="flex items-center gap-3">
            ...
            <input type="checkbox" id="is_active" name="is_active" value="1" @checked($invoice?->is_active ?? true)
                   class="w-4 h-4 rounded bg-vd-form border-vd-formBorder text-vd-primary focus:ring-vd-primary focus:ring-offset-0" />
