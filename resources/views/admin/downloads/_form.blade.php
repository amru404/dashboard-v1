...
<div class="grid gap-5 lg:grid-cols-2">
    <div>
        ...
        <select id="product_id" name="product_id" class="madani-input mt-2" required>
            ...
            @foreach ($productOptions as $option)
                <option value="{{ $option['id'] }}" @selected((string) old('product_id', $downloadItem->product_id) === (string) $option['id'])>
    ...
    <div>
        <x-form-label for="user_id" value="Customer restriction" />
        <select id="user_id" name="user_id" class="madani-input mt-2">
        ...
        <p class="mt-2 text-xs text-madani-muted">Leave blank for all customers entitled to the selected product.</p>
    ...
    <div class="rounded-xl border border-madani-border bg-madani-ghost p-4 lg:col-span-2">
        ...
        <input id="file_upload" name="file_upload" type="file" class="madani-input mt-2">
