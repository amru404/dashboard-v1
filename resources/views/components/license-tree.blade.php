...
<div class="space-y-6">
    @forelse ($products as $product)
        <section x-data="{ open: true }" 
            ...
            <button type="button" @click="open = !open" class="w-full flex items-center justify-between gap-4 px-6 py-4 bg-[#071422] hover:bg-[#0a1d31] transition-colors">
                <div class="flex-1 min-w-0 text-left">
                    ...
                    <div class="flex items-center gap-3 text-xs text-gray-400">
                        ...
                        @endphp
                        @if ($childrenWithLicenses->count() > 0)
                            <span>•</span>
                            <span>{{ $childrenWithLicenses->count() }} {{ Str::plural('module', $childrenWithLicenses->count()) }}</span>
                        @endif
                    </div>
                </div>
            ...
            {{-- Product Content --}}
            <div x-show="open" x-collapse class="border-t border-vd-border bg-vd-card p-6">
                ...
                @if ($product->licenses->isEmpty() && $childrenWithLicenses->isEmpty())
