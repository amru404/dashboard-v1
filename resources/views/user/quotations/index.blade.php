...
<div class="mb-8">
    ...
    <p class="text-base text-gray-300">
        Download your quotations and price documents.
    </p>
</div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-[#2a3f5f] bg-[#0f1829]">
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Quotation Name</th>
            ...
            <tbody class="divide-y divide-[#2a3f5f]">
                @forelse ($quotations as $quotation)
                    <tr class="hover:bg-white/5 transition-colors">
                        <td class="px-6 py-4 text-white font-medium">{{ $quotation->display_name }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-gray-400">
                            @if ($quotation->file_path)
                                <div class="text-sm">
                                    <p class="text-white font-medium">{{ $quotation->original_filename }}</p>
                                    <p class="text-xs text-gray-500 mt-1">{{ number_format($quotation->file_size / 1024 / 1024, 1) }}MB</p>
                                </div>
                            @else
                        ...
                        <td class="px-6 py-4 text-gray-400 text-sm">
                            @if ($quotation->download_expired_at)
                                @if ($quotation->download_expired_at->isPast())
                                    <span class="text-red-400 font-medium">Expired</span>
                                    <p class="text-xs text-gray-500 mt-1">{{ $quotation->download_expired_at->format('M j, Y') }}</p>
                                @else
                                    <p>{{ $quotation->download_expired_at->format('M j, Y') }}</p>
                                @endif
                            @else
                                <p>No limit</p>
                            @endif
                        ...
                        <td class="px-6 py-4 text-center">
                            @if ($quotation->file_path)
                                ...
                                @else
                                    <a href="{{ route('user.quotations.download', $quotation) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-vd-primary/20 hover:bg-vd-primary/30 text-vd-primary font-semibold text-xs border border-vd-primary/30 transition-colors">
                                        Download
                                    </a>
                                @endif
                            @else
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-400">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm font-medium text-white">No quotations available</p>
                                <p class="text-xs text-gray-400 mt-1">Check back later for new quotations</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
